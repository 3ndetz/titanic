...
Проверим, запустим пару разных экспериментов через dvc exp run train --name ...

Так как только 1 степ логируется (т.к. мы используем fit в обучении, для простоты и унификации пайплайнов), на графиках показывается только 1 точка (но даже и её не видно из-за особенностей matplotlib).
...
Вот так: (сам сделал график в png и добавил через dvclive.log_image, он вставил в отчёт)

![alt text](images/hw6/image-6.png)

Но мне этот способ не нравится, хочу чтобы всё собиралось из чисел, поэтому меняю на `live.log_sklearn_plot`, который сохранит исходные числа, а не png. Но в отчёте их больше не будет, ну и ладно. Зато хорошо видно в `>dvc show plots`

![alt text](images/hw6/image-7.png)

Сделаем сравнительную таблицу экспериментов.

`dvc exp show --md > dvclive/experiments_table.md`

Сгенерировалась, пару экспериментов видно и сравнительные метрики тоже.

![alt text](images/hw6/image-8.png)
...
Таким образом, всё готово!
    ...
    * Создать сравнительные таблицы экспериментов
      * Есть
    * Настроить автоматическую генерацию отчетов
      * Есть, они генерятся вместе с dvc exp run и появляются в dvclive/
