...
Комментарий: нет отчетов об экспериментах (должны выгружаться в mkdocs GHIO)
...
Отчёт опубликовался через пару минут:

![alt text](images/hw6_fix/image-4.png)

Всё готово! Отчёты генерируются и в случае, когда нам нужно опубликовать, 1 командой сразу деплоятся в доку на GHIO.
