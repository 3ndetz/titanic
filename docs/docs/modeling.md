...
`dvc metrics diff` покажет в упрощённом формате разницу метрик экспериментов.

Если есть Visual Studio Code, можно установить [DVC Extension](https://marketplace.visualstudio.com/items?itemName=Iterative.dvc) и использовать его для удобного просмотра экспериментов.

Подробнее можно посмотреть в документации расширения.

Например, можно вызвать удобное окно экспериментов, где можно сразу строить графики:

`>dvc show experiments`

(команда вводится в командной палитре VS Code)
