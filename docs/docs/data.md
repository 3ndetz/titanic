...
- [Data](#data)
  ...
  - [Идеальный пайплайн обработки данных](#идеальный-пайплайн-обработки-данных)
...
├── external/          <- Данные из/для сторонних источников
...
В `raw` и `external` допускается `csv`, в остальных используем `parquet` для эффективности.
...
## Идеальный пайплайн обработки данных

1. Сырые данные кладутся в `data/raw/`.
2. Запускается пайплайн клининга данных, и они попадают в `data/interim/`.
   Сейчас этот этап пропущен.
3. Генерим новые фичи, и они попадают в `data/features/`.
   Сейчас этот этап пропущен.
4. Соединяем все данные, энкодируем, в общем готовим к финальному обучению моделей, и кладём в `data/processed/`.
5. Обучаем модели на обработанных данных, сохраняем чекпоинты в `data/models/`.
6. Делаем предсказания на тестовых данных и сохраняем результаты в `data/external/`.
   Сейчас этот этап пропущен.
